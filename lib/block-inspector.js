...
(function () {
  ...
  async function enrichFromRest(options) {
    ...
    try {
      // `?context=edit` requires X-WP-Nonce. Content scripts can't read
      // the page-context `window.wpApiSettings` directly, so we scan the
      // DOM surface that mirrors it (inline scripts + data-* attrs).
      const nonce = globalThis.WPRest.findNonceInDocument(document);
      if (!nonce) return; // No nonce → request would 401, skip silently.
